...
def add_vehicle(info: VehicleInfo):
    try:
        ...
        return {"message": f"Thêm biển số thành công", "data": info}
...
@app.put("/vehicles/{plate}")
def update_vehicle(plate: str, vehicle: VehicleInfo):
    if plate != vehicle.plate:
        raise HTTPException(status_code=400, detail="Plate in URL and body must be the same")

    updated_rows = update_vehicle_in_db(vehicle)
    if updated_rows == 0:
        raise HTTPException(status_code=404, detail="Vehicle not found")

    return {"message": f"Cập nhật biển số thành công"}

@app.post("/login")
def login(user: UserLogin):
    db_user = get_user_by_username(user.username)
    if db_user and db_user[2] == user.password:
        return {
            "id": db_user[0],
            "username": db_user[1],
            "role": db_user[3],
